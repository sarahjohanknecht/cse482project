...
fp = open('sentiment-testing.csv')  # file with classified twitter data for every tweet
csvFile = open('aggregatedData-final.csv', 'a')  # file to store the aggregated data per state per day
...
#calculate the sentiment per state per day
for line in reader:
    date = line[1]
    state = line[0]
    sentiment = line[2]
